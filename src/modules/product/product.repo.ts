...
export class ProductRepo {
  ...
  async create({
    ...
    const { skus: providedSkus, categories, variants, ...productData } = data

    // Xác định SKUs cần tạo
    let skusToCreate
    if (providedSkus && providedSkus.length > 0) {
      // Nếu không có variants, chỉ lấy SKU đầu tiên được cung cấp
      if (!variants || variants.length === 0) {
        skusToCreate = [providedSkus[0]]
      } else {
        // Nếu có variants, sử dụng tất cả SKUs đã cung cấp
        skusToCreate = providedSkus
      }
    } else {
      // Tạo SKU mặc định nếu không có variants hoặc skus
      // Lấy hình ảnh đầu tiên nếu có
      const defaultImage = productData.images && productData.images.length > 0 ? productData.images[0] : ''

      // Tạo SKU mặc định với tên sản phẩm làm value
      skusToCreate = [
        createDefaultSKU(
          productData.name,
    ...
    const product = await this.prismaService.product.create({
      data: {
        createdById,
        ...productData,
        // Đảm bảo variants là mảng rỗng nếu không được cung cấp
        variants: variants || [],
        categories: {
    ...
    return product as unknown as GetProductDetailResType
  }
